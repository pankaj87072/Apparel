...
const ApparelRecyclingMVP = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if(data.ok)console.log("wokring fine")
  };
  useEffect(() => {
    if (showAlert) {
    ...
    <div className="container mx-auto px-4 py-8 max-w-2xl">
        ...
        {submissions.length === 0 ? (
          ...
          <ul className="space-y-2">
            {submissions.map((submission) => (
              <li key={submission.id} className="flex justify-between items-center bg-gray-100 p-3 rounded">
      ...
      <div className="bg-blue-100 rounded-lg p-4">
        ...
        >
          ...
          <svg 
            className={`ml-2 h-5 w-5 transform transition-transform duration-200 ${isInfoOpen ? 'rotate-180' : ''}`}
